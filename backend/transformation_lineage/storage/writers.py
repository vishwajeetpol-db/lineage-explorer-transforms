...
# ── Explicit schemas (prevent CANNOT_DETERMINE_TYPE on all-None columns
#    and type-mismatch errors on Delta merges) ────────────────────────
...
def write_parse_metrics_batch(
    ...
    `metrics` is a sequence of (extraction_id, parse_dict). Replaces the prior
    one-write-per-artifact pattern (N Delta transactions) with a single write.
    """
    if not metrics:
        return
    now = datetime.now(timezone.utc)
