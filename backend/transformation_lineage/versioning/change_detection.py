...
def is_new_content_version(spark: SparkSession, versions_table_fqn: str, extraction_id: str, sha256: str) -> bool:
    ...
    NOTE: Prefer batch_check_versions() for multiple artifacts — this is kept
    for backward compatibility but is O(N) queries for N artifacts.
    """
    try:
...
def batch_check_versions(
    ...
    Cost savings: O(1) query instead of O(N) queries for N artifacts.
    Uses window function to get latest hash per extraction_id efficiently.
    """
    if not sha_map:
        return set()

    try:
        # Get the latest content_sha256 for each extraction_id in one query
        ...
        # Table doesn't exist yet — all artifacts are new
        return set(sha_map.keys())

    # Build lookup: extraction_id -> last known sha
    known_shas = {row["extraction_id"]: row["latest_sha"] for row in latest_versions}

    # An artifact is "new" if:
    #   1. It has never been seen before (not in known_shas)
...
def batch_record_versions(
    ...
    *,
    ...
    Cost savings: 1 Delta write operation instead of N.
    """
    if not sha_map:
        return
